...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        if len(legal_actions) == 0:
            ...
            for action in legal_actions:
                if abs(self.getQValue(state, action) - MAX) < 1e-9:
                    return action
    ...
    def getAction(self, state):
        ...
        if len(legal_actions) == 0:
            ...
            if flipCoin(self.epsilon):
                # the random is smaller then randomly choose one
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        # wrong ! : for i in weights
        for i in feature_vector:

            ret += weights[i]*feature_vector[i]
        return ret
    ...
    def update(self, state, action, nextState, reward):
        ...
        r = reward
        gama = self.discount
        # MAX = float('-inf') # maxQ(s', a')
        # for action in self.getLegalActions(nextState):
        #     v = self.getQValue(nextState, action)
        #     if v > MAX:
        #         MAX = v
        # q_value = self.getQValue(state, action)
        MAX =  self.getValue(nextState)
        difference = (r+gama*MAX) - self.getQValue(state,action)
        feature_vector = self.featExtractor.getFeatures(state, action)
        for i in feature_vector:
